...
# native ScanNet color resolution the intrinsic_color.txt matrices refer to
NATIVE_W, NATIVE_H = 1296, 968

# precomputed alignment between each map scan (_01) and its query scan (_00)
ALIGNMENT_FILE = Path(__file__).resolve().parent / "preprocessing" / "db_to_query_transforms.txt"


def load_db_to_query_transforms(filepath):
    """Per map scan, the 4x4 map-to-query transform, read from the precomputed alignment
    file (one scan id plus 16 row-major values per line)."""
    transforms = {}
    with open(filepath) as f:
        for line in f:
            ...
            if len(parts) != 17:
                continue
            transforms[parts[0]] = np.array(parts[1:], dtype=np.float64).reshape(4, 4)
    return transforms
...
def isPoseValid(pose):
    ...
    is_rotation_valid = (
        np.isclose(np.matmul(R_matrix, R_matrix.T), identity, atol=1e-3)
...
def load_frame_idxs(data_split_dir, scan_id, skip=None):
    """String frame indices ("0", "25", ...) in temporal order, every skip-th frame."""
    num_frames = len(glob(osp.join(data_split_dir, scan_id, "color", "*.jpg")))
    step = 1 if skip is None else skip
    return [f"{frame_idx:d}" for frame_idx in range(0, num_frames, step)]
...
def load_frame_poses(data_split_dir, scan_id, skip=None):
    ...
    for frame_idx in frame_idxs:
        pose_file = osp.join(pose_folder, f"{frame_idx}.txt")
        pose = np.loadtxt(pose_file)
        if not isPoseValid(pose):
            continue  # frames with an invalid (nan/inf) GT pose are dropped
...
def load_frame_intrinsics(data_split_dir, scan_id, sensor="color"):
    if sensor == "color":
        ...
        rgb_intrinsic = np.ascontiguousarray(np.loadtxt(intrinsic_file)[:3, :3])
        # some scans ship depth intrinsics in intrinsic_color.txt. Fall back to an
        # approximate color intrinsic matrix for those.
        ...
        depth_intrinsic = np.ascontiguousarray(np.loadtxt(intrinsic_file)[:3, :3])
...
def scenegraphfusion2scan3r(scan_id, prediction_folder, edge2idx, class2idx, cfg):
    ...
    # drop segments below the size threshold
    segment_ids = np.unique(segments_pd)
    segment_ids = segment_ids[segment_ids != 0]
    segments_pd_filtered = []
    ...
    points_pd = points_pd[np.where(points_pd_mask)[0]]
    segments_pd = segments_pd[np.where(points_pd_mask)[0]]
    ...
    points_pd = points_pd[np.where(segments_ids_pc_mask)[0]]
    segments_pd = segments_pd[np.where(segments_ids_pc_mask)[0]]
    verts = []
...
def filter_merge_same_part_pc(segments_pd, objects, relationships):
    ...
    for relationship in relationships:
        if relationship[-1] != NAME_SAME_PART:
            filtered_relationships.append(relationship)
        elif (
            relationship[-1] == NAME_SAME_PART
            and instanceid2objname[int(relationship[0])] == instanceid2objname[int(relationship[1])]
        ):
    ...
    for same_part in same_parts:
        ...
        for part_segment_id in same_part[1:]:
            ...
            for idx, (sub, ob, rel_id, _rel_name) in enumerate(filtered_relationships):
                sub = int(sub)
                ob = int(ob)
                rel_id = int(rel_id)
...
def process_ply_data2scan3r(scan_id, ply_data, rels_dict, objs_dict, rel2idx, cfg):
    ...
    for index, v in enumerate(s):
        s[index] = object_id2idx[v]

    for index, v in enumerate(o):
        o[index] = object_id2idx[v]
...
def make_bow_vector(sentence, word_2_idx):
    ...
    for word in sentence:
        if word not in word_2_idx:
            raise ValueError(f"unknown relationship name: {word}")
